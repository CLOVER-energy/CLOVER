...
class Finance:
    ...
    def get_diesel_cost(self, diesel_size, year=0):
        ...
        annual_reduction = (
            0.01 * self.finance_inputs.loc["Diesel generator cost decrease"]
        )
    ...
    def get_installation_cost(self, PV_array_size, diesel_size, year=0):
        ...
        """
        PV_installation = (
            PV_array_size * self.finance_inputs.loc["PV installation cost"]
        )
        annual_reduction_PV = (
            0.01 * self.finance_inputs.loc["PV installation cost decrease"]
        )
        diesel_installation = (
            diesel_size * self.finance_inputs.loc["Diesel installation cost"]
        )
        annual_reduction_diesel = (
            0.01 * self.finance_inputs.loc["Diesel installation cost decrease"]
        )
        return (
    ...
    def get_misc_costs(self, PV_array_size, diesel_size):
        ...
        Outputs:
            Undiscounted cost
        """
        misc_costs = (PV_array_size + diesel_size) * self.finance_inputs.loc[
            "Misc. costs"
        ]
        return misc_costs
    ...
    def get_PV_OM(self, PV_array_size, start_year=0, end_year=20):
        ...
        total_daily_cost = pd.DataFrame(
            [PV_OM_cost_daily] * (end_year - start_year) * 365
        )
    ...
    def get_diesel_OM(self, diesel_size, start_year=0, end_year=20):
        ...
        """
        diesel_OM_cost = (
            diesel_size * self.finance_inputs.loc["Diesel O&M"]
        )  # $ per year
