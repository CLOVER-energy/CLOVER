...
class GHGs:
    def __init__(self):
        ...
        self.GHG_filepath = os.path.join(
            self.location_filepath, "Impact", "GHG inputs.csv"
        )
        self.GHG_inputs = pd.read_csv(
            self.GHG_filepath, header=None, index_col=0
        ).round(decimals=3)[1]
        self.finance_filepath = os.path.join(
            self.location_filepath, "Impact", "Finance inputs.csv"
        )
        self.finance_inputs = pd.read_csv(
            self.finance_filepath, header=None, index_col=0
        ).round(decimals=3)[1]
        self.inverter_inputs = pd.read_csv(
            os.path.join(
                self.location_filepath,
                "Load",
                "Device load",
                "yearly_load_statistics.csv",
            ),
            index_col=0,
        )
    ...
    def get_installation_GHGs(self, PV_array_size, diesel_size, year=0):
        ...
        diesel_installation = (
            diesel_size * self.GHG_inputs.loc["Diesel installation GHGs"]
        )
        annual_reduction_diesel = (
            0.01 * self.GHG_inputs.loc["Diesel installation GHG decrease"]
        )
        return (
            PV_installation * (1.0 - annual_reduction_PV) ** year
            + diesel_installation * (1.0 - annual_reduction_diesel) ** year
        )
    ...
    def get_total_equipment_GHGs(
        self, PV_array_size, storage_size, diesel_size, year=0
    ):
        """
        Function:
    ...
    def get_inverter_GHGs(self, start_year, end_year):
        ...
        inverter_info["Inverter GHGs (kgCO2/kW)"] = [
            self.GHG_inputs.loc["Inverter GHGs"]
    ...
    def get_kerosene_GHGs_mitigated(
    ...
    def get_PV_OM(self, PV_array_size, start_year=0, end_year=20):
        ...
        """
        return (
            PV_array_size * self.GHG_inputs.loc["PV O&M GHGs"] * (end_year - start_year)
        )
    ...
    def get_storage_OM(self, storage_size, start_year=0, end_year=20):
        ...
        return (
            storage_size
            * self.GHG_inputs.loc["Storage O&M GHGs"]
            * (end_year - start_year)
        )
    ...
    def get_diesel_OM(self, diesel_size, start_year=0, end_year=20):
        ...
        return (
            diesel_size
            * self.GHG_inputs.loc["Diesel O&M GHGs"]
            * (end_year - start_year)
        )
