...
class EnergySystemOld:
    ...
    # =============================================================================
    def simulation(
        self, start_year=0, end_year=4, PV_size=10, storage_size=10, **options
    ):
        """
        Function:
        ...
        #   Get input profiles
        input_profiles = self.get_storage_profile(
            start_year, end_year, PV_size, **options
        )
        ...
        max_energy_throughput = (
            storage_size * self.energy_system_inputs[1]["Battery cycle lifetime"]
        )
        initial_storage = (
            storage_size * self.energy_system_inputs[1]["Battery maximum charge"]
        )
        max_storage = (
            storage_size * self.energy_system_inputs[1]["Battery maximum charge"]
        )
        min_storage = (
            storage_size * self.energy_system_inputs[1]["Battery minimum charge"]
        )
            ...
            if t == 0:
                ...
                if battery_energy_flow >= 0.0:  # Battery charging
                    new_hourly_storage = hourly_storage[t - 1] * (
                        ...
                        battery_energy_flow,
                        battery_C_rate_in * (max_storage - min_storage),
                    )
                else:  # Battery discharging
                    new_hourly_storage = hourly_storage[t - 1] * (
                        1.0 - battery_leakage
                    ) + (1.0 / battery_eff_out) * max(
                        battery_energy_flow,
                        (-1.0) * battery_C_rate_out * (max_storage - min_storage),
                    )
        ...
        print(
            ...
            + "{0:.2f}".format(
                (time_delta.microseconds * 0.000001) / float(end_year - start_year)
            )
            + " seconds per year"
        )
    ...
    def get_storage_profile(self, start_year=0, end_year=4, PV_size=10, **options):
        ...
        if self.scenario_inputs[1]["Distribution network"] == "DC":
            PV_generation = (
                self.energy_system_inputs[1]["DC to DC conversion"] * PV_generation
            )
            transmission_eff = self.energy_system_inputs[1][
                "Transmission efficiency DC"
            ]
        #            grid_conversion_eff = self.energy_system_inputs[1]['AC to DC conversion']

        if self.scenario_inputs[1]["Distribution network"] == "AC":
            PV_generation = (
                self.energy_system_inputs[1]["DC to AC conversion"] * PV_generation
            )
            transmission_eff = self.energy_system_inputs[1][
                "Transmission efficiency AC"
            ]
        ...
        if self.scenario_inputs[1]["Prioritise self generation"] == "N":
            ...
            grid_energy = pd.DataFrame(load_energy.values) * pd.DataFrame(
                grid_status.values
            )  # as needed for load
            remaining_profile = (grid_energy <= 0) * load_energy
            #   Then take energy from PV
            storage_profile = pd.DataFrame(
                renewables_energy.values - remaining_profile.values
            )
            renewables_energy_used_directly = pd.DataFrame(
    ...
    def get_PV_generation(self, **options):
        ...
        return pd.read_csv(
            os.path.join(
                self.generation_filepath, "PV", "solar_generation_20_years.csv"
            ),
            header=None,
            index_col=0,
        )
    ...
    def get_grid_profile(self, **options):
        ...
        return pd.read_csv(
            os.path.join(
                self.generation_filepath, "Grid", grid_type + "_grid_status.csv"
            ),
            index_col=0,
        )
