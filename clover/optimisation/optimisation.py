...
class OptimisationOld:
    ...
    def optimisation_step(
        ...
        """
        iteration_results = self.simulation_iteration(
            PV_sizes, storage_sizes, previous_systems, start_year
        )
    ...
    def single_line_simulation(
        ...
        if float(potential_system["Initial storage size"]) == storage_size_max:
            ...
            while iteration_PV_size >= PV_size_min:
                simulation = Energy_System().simulation(
    ...
    def simulation_iteration(
        ...
        largest_system = pd.DataFrame(
            {
                "PV size (max)": PV_size_max,
    ...
    def simulation_technical_appraisal(self, simulation):
        ...
        total_renewables_used = np.sum(
            simulation_results["Renewables energy used (kWh)"]
        )
    ...
    def simulation_environmental_appraisal(
        ...
        total_system_GHGs = (
            equipment_GHGs + connections_GHGs + OM_GHGs + diesel_GHGs + grid_GHGs
        )
        #   Return outputs
    ...
    def check_threshold(self, system_appraisals):
        ...
        if threshold_criterion in self.maximum_criteria:
            sufficient_systems = system_appraisals[
                system_appraisals[threshold_criterion] <= threshold_value
            ]
