...
def plot_outputs(  # pylint: disable=too-many-locals, too-many-statements
    ...
    with tqdm(
        ...
        plt.plot(range(365), solar_daily_sums[0], color="C7")
        ...
        max_value = np.max(np.max(diesel_energy), np.max(grid_energy), np.max(renewable_energy), np.max(storage_energy))

        fig, ([ax1, ax2], [ax3, ax4]) = plt.subplots(2, 2)  # ,sharex=True, sharey=True)
        sns.heatmap(
            renewable_energy, vmin=0.0, vmax=4.0, cmap=green_blended, cbar=True, ax=ax1
        )
        ax1.set(
            xticks=range(0, 25, 6),
        ...
        sns.heatmap(
            storage_energy, vmin=0.0, vmax=4.0, cmap=light_green_blended, cbar=True, ax=ax2
        )
        ax2.set(
            xticks=range(0, 25, 6),
            xticklabels=range(0, 25, 6),
        ...
        sns.heatmap(grid_energy, vmin=0.0, vmax=4.0, cmap=light_orange_blended, cbar=True, ax=ax3)
        ax3.set(
            xticks=range(0, 25, 6),
            xticklabels=range(0, 25, 6),
            yticks=range(0, 365, 60),
        ...
        sns.heatmap(diesel_energy, vmin=0.0, vmax=4.0, cmap=orange_blended, cbar=True, ax=ax4)
        ax4.set(
            xticks=range(0, 25, 6),
            xticklabels=range(0, 25, 6),
            yticks=range(0, 365, 60),
