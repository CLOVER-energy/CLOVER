...
def calculate_true_solar(
    ...
    for fcst_month in range(total_num_months):
        ...
        for obs_month in range(fcst_month + 1, total_num_months):
            ...
            rms = np.sqrt(np.mean((x[:num_days])**2)) - np.sqrt(np.mean((y[:num_days])**2))
    ...
    for fcst_month in range(total_num_months):
        ...
        best_obs_month = int(np.nanargmin(np.abs(candidate_corr - target_corr) + np.abs(candidate_rms)))

        # replace forecast with observation
        num_fcst_hrs = len(monthly_idx[fcst_month])*24
        fcst_hrs = range(min(monthly_idx[fcst_month])*24, (max(monthly_idx[fcst_month])+1)*24)
        obs_hrs = range(min(monthly_idx[best_obs_month])*24, (max(monthly_idx[best_obs_month])+1)*24)
        # num_days = len(monthly_idx[fcst_month])

        true_solar_profile.iloc[fcst_hrs] = pv_produced.iloc[obs_hrs[:num_fcst_hrs]].to_numpy()
